...
import {
  useCreateVodafoneNumbersWithdrawal,
  useUpdateVodafoneNumbersWithdrawal,
} from "./mutations";

export default function VodafoneComponentWithdrawal() {
  ...
  const updateVodafoneNumbersMutation = useUpdateVodafoneNumbersWithdrawal();
  const createVodafoneNumbersMutation = useCreateVodafoneNumbersWithdrawal();

  function handleSubmit(values: {
    transfers: { phoneNumber: string; amount: string }[];
  }) {
    try {
      if (!values.transfers[0].phoneNumber || !values.transfers[0].amount) {
        toast({
          variant: "destructive",
                    ...
                    <FormItem className="text-left">
                      <FormControl>
                        <PhoneInput
                          ...
                          inputClass={`border-input !w-full p-6 placeholder:text-muted-foreground text-primary rounded-2xl shadow-[rgba(0,_0,_0,_0.15)_0px_2px_8px] ${
